 ...
 * mapping modifications (such as fallocate, hole punching, or page allocation)
 ...
 *
 * 1) Readers: `kvm_gmem_get_folio()` acts as the central gatekeeper, acquiring the
 *    SRCU read lock around its checks and allocations. Fallocate acquires it
 *    nestedly (safe due to SRCU's re-entrant design) to protect the punch-hole path.
 * 2) Writer: This function `kvm_gmem_freeze` toggles the `GUEST_MEMFD_F_MAPPING_FROZEN`
 *    flag and invokes `synchronize_srcu(&kvm_gmem_freeze_srcu)` to block/drain all
 *    active allocation and punch-hole threads before returning, guaranteeing a static
 *    mapping.
 *
 * Global vs. Per-Inode SRCU Design Decision:
 * A single system-wide global static `srcu_struct` is used instead of a per-inode
 * SRCU structure to completely prevent unprivileged users from exhausting the host's
 * per-CPU memory allocator. Because `init_srcu_struct()` allocates per-CPU memory
 * via `alloc_percpu()`, which is not accounted by memory cgroups (memcg), a per-inode
 * SRCU structure would allow a tenant to bypass cgroup limits and trigger a system-wide
 * Out-of-Memory (OOM) crash simply by spawning a large number of guest_memfd file
 * descriptors (bounded only by RLIMIT_NOFILE).
 *
 * Because the read-side page fault and fallocate paths do not hold any locks when
 * acquiring the SRCU read lock, and because the writer (`kvm_gmem_freeze()`) does
 * not hold VFS locks during `synchronize_srcu()`, sharing the global structure is
 * completely deadlock-free. Furthermore, because SRCU read-side critical sections
 * are lockless and utilize per-CPU counters, there is zero cacheline bouncing or
 * cross-VM performance interference under high fault rates.
 *
 ...
 *
 * WARNING: If Freeze is successful, it is expected either the VM is paused or
 * VMM will take care of pausing it on VM_EXIT when page fault fails in KVM.
 */
